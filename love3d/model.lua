...
local newMatrix = require(love3d.path .. "/matrices")
local loadObjFile = require(love3d.path .. "/objloader")
local collisions = require(love3d.path .. "/collisions")
local vectors = require(love3d.path .. "/vectors")
local camera = require(love3d.path .. "/camera")
...
function model:upsideDown(isFlipped)
...
-- using euler angles
function model:setEulerRotation(rx,ry,rz)
    self.rotation[1] = rx
    self.rotation[2] = ry
    self.rotation[3] = rz
    self.rotation[4] = nil
...
function model:setDegreeRotation(x,y,z,angle)
...
function model:compress()
    print("[love3d warning] Compression requires FFI!\n" .. debug.traceback())
end
