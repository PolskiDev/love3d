-- written by groverbuger for love3d
-- september 2021
-- MIT license

local vectors = require(love3d.path .. "/vectors")
...
-- Updated Primitive Collision (Do not use it!)
function collisions.hasLevelCollided(sizeof, x,y,z)
    local floor = sizeof
    if love3d.world.getCameraX() > x
    and love3d.world.getCameraX() < x+floor then
        return true
    elseif love3d.world.getCameraY() > y
    and love3d.world.getCameraY() < y+floor then
        return true
    elseif love3d.world.getCameraZ() > z
    and love3d.world.getCameraZ() < z+floor then
        return true
    else
        return false
    end
...
-- Object Standard Collision (Recommended)
function collisions.hasEntityCollided(length, x1,y1,z1, x2,y2,z2)
    local floor = length
    if x1 >= x2-floor and x1 <= x2+floor and
    y1 >= y2-floor and y1 <= y2+floor and
    z1 >= z2-floor and z1 <= z2+floor
    then
        return true
    else
        return false
...
-- New Standard Collision (Recommended)
function collisions.hasCameraCollided(length, x,y,z)
    local floor = length
    if love3d.world.getCameraX() >= x-floor and
    love3d.world.getCameraX() <= x+floor and
    love3d.world.getCameraY() >= y-floor and
    love3d.world.getCameraY() <= y+floor and
    love3d.world.getCameraZ() >= z-floor and
    love3d.world.getCameraZ() <= z+floor
    then
        return true
    else
        return false
